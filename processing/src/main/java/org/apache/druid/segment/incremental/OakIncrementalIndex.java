...
public class OakIncrementalIndex extends IncrementalIndex<BufferAggregator>
{
  ...
  protected BufferAggregator[] initAggs(AggregatorFactory[] metrics,
                                        ...
                                        boolean concurrentEventAdd)
  {
    this.aggsManager = new AggsManager(metrics);
                                        ...
                                        boolean skipMaxRowsInMemoryCheck) throws IndexSizeExceededException
  {
    ...
    final RowContext ctx = new RowContext(rowContainer, row);
    facts.putIfAbsentAggIfPresent(ctx, key);
  ...
  private static class RowContext
  {
    ...
    RowContext(ThreadLocal<InputRow> rowContainer, Row row)
    {
  ...
  static class AggsManager
  {
    ...
    public AggsManager(AggregatorFactory[] metrics)
    {
    ...
    public void initValue(RowContext ctx, OakBuffer buffer)
    {
      OakUnsafeDirectBuffer unsafeBuffer = (OakUnsafeDirectBuffer) buffer;
      initValue(ctx, unsafeBuffer.getByteBuffer(), unsafeBuffer.getOffset());
    }

    public void initValue(RowContext ctx, ByteBuffer aggBuffer, int aggOffset)
    {
    ...
    public void aggregate(RowContext ctx, OakBuffer buffer)
    {
      OakUnsafeDirectBuffer unsafeBuffer = (OakUnsafeDirectBuffer) buffer;
      aggregate(ctx, unsafeBuffer.getByteBuffer(), unsafeBuffer.getOffset());
    }

    public void aggregate(RowContext ctx, ByteBuffer aggBuffer, int aggOffset)
    {
      ctx.setRow();
  ...
  {
    private final OakMap<IncrementalIndexRow, RowContext> oak;
     ...
     */
    class OakValueSerializer implements OakSerializer<RowContext>
    {
      @Override
      public void serialize(RowContext ctx, OakScopedWriteBuffer buffer)
      {
        OakUnsafeDirectBuffer unsafeBuffer = (OakUnsafeDirectBuffer) buffer;
        aggsManager.initValue(ctx, unsafeBuffer.getByteBuffer(), unsafeBuffer.getOffset());
      }

      @Override
      public RowContext deserialize(OakScopedReadBuffer buffer)
      {
      ...
      @Override
      public int calculateSize(RowContext row)
      {
    ...
    {
      OakMap<IncrementalIndexRow, RowContext> tmpOakMap = descending ? oak.descendingMap() : oak;
    ...
    public Iterable<IncrementalIndexRow> timeRangeIterable(boolean descending, long timeStart, long timeEnd)
    {
      return () -> {
        ...
        OakMap<IncrementalIndexRow, RowContext> subMap = oak.subMap(from, true, to, false, descending);
     ...
     */
    public boolean putIfAbsentAggIfPresent(RowContext ctx, IncrementalIndexRow key)
    {
      if (rollup) {
      ...
      return oak.zc().putIfAbsentComputeIfPresent(key, ctx, buffer -> aggsManager.aggregate(ctx, buffer));
    }
